# Ablation Config: True Baseline (No Auxiliary Losses)
# Pure CrossEntropy only - to establish baseline performance
...
# Model configuration - NO auxiliary losses
model = dict(
    type='EncoderDecoder',
    pretrained=None,
    backbone=dict(
    ...
    decode_head=dict(
        ...
        # BASELINE: All auxiliary losses disabled
        boundary_lambda=0.0,
        proto_lambda=0.0,
        contrastive_lambda=0.0,
        boundary_mode='sobel',
        boundary_loss_mode='binary',
        # Still need dummy configs for init, but losses are disabled
        dynamic_anchor=dict(
...
# 60k iterations to match ablation variants
...
work_dir = './work_dirs/openearthmap/ablation/unetformer_resnext101_ce_only'
gpu_ids = range(0, 1)
