...
class GlobalLocalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 3, 5, 2, 4, 6, 1).reshape(
            B * (Hp // ws) * (Wp // ws), N, ws * ws, head_dim)

        qkv = self.qkv(x)
        qkv = qkv.reshape(
            qkv.shape[0], qkv.shape[1], qkv.shape[2],
            3, self.num_heads, head_dim).permute(3, 0, 4, 1, 2, 5)
        ...
        x = self.proj(x)
        x = x.reshape(
            B, Hp // ws, Wp // ws, ws, ws, N, head_dim
        ).permute(0, 5, 1, 3, 2, 4, 6).reshape(B, N * head_dim, Hp, Wp)
...
class Block(nn.Module):
    def __init__(self, dim=256, num_heads=16, mlp_ratio=4., qkv_bias=False,
        ...
        self.attn = GlobalLocalAttention(
            ...
            window_size=window_size, attn_drop=attn_drop, proj_drop=drop)
...
class WF(nn.Module):
    def __init__(self, in_channels=128, decode_channels=128, eps=1e-8):
        ...
        self.weights = nn.Parameter(torch.ones(2, dtype=torch.float32) * eps)

    def forward(self, x, res):
        x = self.pre_conv(x)
        return self.weights[0] * x + self.weights[1] * res
...
class UNetFormerDecoderHead(BaseModule):
    """UNetFormer Decoder head for MMSeg v0.16.0.

    Wraps the reference UNetFormer Decoder with the BaseDecodeHead-compatible
    interface so it works inside EncoderDecoder.

    Args:
        encoder_channels (tuple): Channel dims of the 4 backbone feature maps.
        decode_channels (int): Decoder hidden channel dimension.
        num_classes (int): Number of segmentation classes.
        dropout (float): Dropout rate.
        window_size (int): Window attention size.
        ignore_index (int): Label index to ignore in loss.
        align_corners (bool): Align corners in interpolation.
        init_cfg (dict): Init config.
    """
